...
const verifyCredentials = async (req, res, next) => {
    ...
    try {
        ...
        const userQuery = await pool.query("SELECT email, hash, role, user_id, active, name, assigned_area, balance FROM login WHERE email = $1", [clientEmail])
        if (!userQuery.rows[0])
            return res.status(403).json("email or password does not match")
        const { email, hash, role, user_id, active, name, assigned_area, balance } = userQuery.rows[0]
        if (!active)
